...
export default function EventDashboardPage() {
  ...
  const downloadQR = () => {
    if (!event) return
    const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${encodeURIComponent(qrCodeUrl)}`
  ...
  const qrCodeUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/scan/${event.qrCode}`
          ...
          <div className="lg:col-span-2">
            <div className="flex justify-between items-center mb-6">
              ...
              <Button onClick={() => setShowCreateForm(!showCreateForm)}>
                ...
                {showCreateForm ? 'Annulla' : 'Nuova Galleria'}
              </Button>
            </div>
            ...
            {showCreateForm && (
              <Card className="p-6 mb-6">
                ...
                <form onSubmit={createGallery} className="space-y-4">
                  <Input
                    placeholder="Nome galleria (es. Ceremony, Reception)"
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {galleries.length === 0 ? (
                <Card className="p-8 text-center md:col-span-2">
                  ...
                  <p className="text-gray-600 mb-4">Crea la prima galleria per iniziare a raccogliere foto</p>
                </Card>
              ) : (
                galleries.map((gallery) => (
                  <Link key={gallery.id} href={`/galleries/${gallery.id}`}>
                    <Card className="p-6 hover:shadow-lg transition-shadow cursor-pointer">
                      <h3 className="font-semibold text-lg mb-2">{gallery.name}</h3>
                      {gallery.description && (
